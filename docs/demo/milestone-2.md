...
Milestone 2 moves the prototype from a single in-memory node to a small static-config cluster with one shared Raft group. The implementation keeps mutating lease commands (`Acquire`, `Renew`, `Release`) behind an explicit Raft boundary, persists local term/vote/log metadata to disk, and uses simple TCP peer RPCs without multiplexing.
