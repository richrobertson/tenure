/**
 * Tenant quota policy and lookup helpers.
 *
 * Quota checks are part of admission: they bound lease TTLs and the number of concurrently active leases a
 * tenant may hold.
 */
...
/** Per-tenant policy limits enforced by the service layer. */
final case class TenantQuotaPolicy(maxActiveLeases: Int, maxTtlSeconds: Long) derives CanEqual

/** Circe codecs for [[TenantQuotaPolicy]]. */
object TenantQuotaPolicy:
  given Codec[TenantQuotaPolicy] = deriveCodec

/**
 * Registry of quota policies.
 *
 * The registry supplies a default policy and optional tenant-specific overrides.
 */
final case class TenantQuotaRegistry(defaultPolicy: TenantQuotaPolicy, tenantOverrides: Map[TenantId, TenantQuotaPolicy]) derives CanEqual:
  /** Returns the effective policy for a tenant, falling back to [[defaultPolicy]]. */
  def policyFor(tenantId: TenantId): TenantQuotaPolicy = tenantOverrides.getOrElse(tenantId, defaultPolicy)

/** Common [[TenantQuotaRegistry]] constructors. */
object TenantQuotaRegistry:
  /** Default v1 quota registry used by in-memory/demo setups. */
  ...
  /** Validates one requested TTL against a supplied effective policy. */
  ...
  /** Validates one active-lease count against a supplied effective policy. */
...
/** Extractor that exposes the error message from a quota-validation result. */
